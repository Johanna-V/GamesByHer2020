...
/*Here we calculate the difference between the x position of the target node and the camera.
If the two are far enough apart, we move the camera towards the target, using the distance 
between the two positions as the ‘speed per second’. The further away from the target the camera 
is, the faster the camera moves.*/
